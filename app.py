import streamlit as st
import pandas as pd
import os
import google.generativeai as genai

# ==========================================
# ⚙️ 初始設定與 API Key
# ==========================================
st.set_page_config(page_title="放射師國考刷題神器 V3.1", layout="wide")

# 安全讀取 API Key
try:
    GOOGLE_API_KEY = st.secrets["GEMINI_API_KEY"]
    genai.configure(api_key=GOOGLE_API_KEY)
    model = genai.GenerativeModel('gemini-1.5-flash') 
except Exception as e:
    st.error("⚠️ 請確認 Streamlit 後台 Secrets 設定正確。")
    st.stop()

# ==========================================
# 📂 資料存取與分頁邏輯
# ==========================================
@st.cache_data
def load_quiz_data():
    excel_file = pd.ExcelFile('exam_db.xlsx') 
    db = {sheet: excel_file.parse(sheet) for sheet in excel_file.sheet_names}
    return db

def load_user_records(data_type):
    file = f"user_{data_type}.csv"
    if os.path.exists(file):
        return pd.read_csv(file).astype(str)
    return pd.DataFrame(columns=['user_id', 'subject', 'year', 'q_num'])

def save_record(data_type, user_id, subject, year, q_num, action="add"):
    df = load_user_records(data_type)
    user_id, subject, year, q_num = str(user_id), str(subject), str(year), str(q_num)
    mask = (df['user_id'] == user_id) & (df['subject'] == subject) & \
           (df['year'] == year) & (df['q_num'] == q_num)
    
    if action == "add" and not df[mask].any().any():
        new_row = pd.DataFrame([[user_id, subject, year, q_num]], columns=df.columns)
        df = pd.concat([df, new_row], ignore_index=True)
    elif action == "remove":
        df = df[~mask]
    df.to_csv(f"user_{data_type}.csv", index=False)

def render_content(content):
    """偵測 .png 顯示圖片，否則顯示文字"""
    content_str = str(content).strip()
    if ".png" in content_str.lower():
        img_path = os.path.join("images", content_str)
        if os.path.exists(img_path):
            st.image(img_path, width=400)
        else:
            st.warning(f"找不到圖片：{content_str}")
    elif content_str != "nan":
        st.write(content_str)

# ==========================================
# 📱 側邊欄控制
# ==========================================
st.sidebar.title("🩺 放射師專屬題庫")
user_id = st.sidebar.selectbox("使用者：", ["614", "941", "Guest"])

try:
    quiz_db = load_quiz_data()
    subject = st.sidebar.radio("選擇科目：", list(quiz_db.keys()))
    df_full = quiz_db[subject].copy()
    
    # 處理年度 nan 問題
    valid_years = sorted([str(y) for y in df_full['年度-期別'].unique() if pd.notna(y) and str(y).lower() != 'nan'])
    selected_year = st.sidebar.selectbox("選擇年度：", ["全部年度"] + valid_years)
    
    mode = st.sidebar.selectbox("練習模式：", ["一般練習", "錯題重刷", "標記題庫"])
    st.sidebar.divider()
    hide_done = st.sidebar.checkbox("✅ 隱藏已答對題目", value=True)
    
except Exception as e:
    st.error(f"讀取失敗：{e}")
    st.stop()

# ==========================================
# 🧠 題目過濾與分頁處理 (核心效能優化)
# ==========================================
df_view = df_full.copy().astype(str)
if selected_year != "全部年度":
    df_view = df_view[df_view['年度-期別'] == selected_year]

# 隱藏已答對
if hide_done:
    history = load_user_records("history")
    history = history[history['user_id'] == user_id]
    df_view = df_view.merge(history, on=['user_id', 'subject', 'year', 'q_num'], how='left', indicator=True)
    df_view = df_view[df_view['_merge'] == 'left_only'].drop(columns=['_merge'])

# 根據模式再過濾 (錯題/標記)
if mode != "一般練習":
    rec_type = "wrong" if mode == "錯題重刷" else "marks"
    recs = load_user_records(rec_type)
    recs = recs[recs['user_id'] == user_id]
    df_view = df_view.merge(recs, on=['user_id', 'subject', 'year', 'q_num'])

# --- 分頁器設定 ---
q_per_page = 5 # 每頁顯示 5 題，這是速度飛快的關鍵
total_q = len(df_view)
total_pages = (total_q // q_per_page) + (1 if total_q % q_per_page > 0 else 0)

if total_pages > 0:
    page = st.sidebar.number_input(f"頁數 (共 {total_pages} 頁)", min_value=1, max_value=total_pages, step=1)
    start_idx = (page - 1) * q_per_page
    df_page = df_view.iloc[start_idx : start_idx + q_per_page]
else:
    st.success("🎉 目前此條件下已無題目囉！")
    df_page = pd.DataFrame()

# ==========================================
# ✍️ 測驗主畫面
# ==========================================
st.title(f"第 {page} 頁 / 共 {total_pages} 頁")
marked_df = load_user_records("marks")

for _, row in df_page.iterrows():
    q_key = f"{subject}_{row['年度-期別']}_{row['題號']}"
    
    with st.container(border=True):
        c_head, c_mark = st.columns([8, 2])
        c_head.markdown(f"#### 第 {row['題號']} 題 ({row['年度-期別']})")
        
        is_m = ((marked_df['user_id'] == user_id) & (marked_df['subject'] == subject) & 
                (marked_df['year'] == row['年度-期別']) & (marked_df['q_num'] == row['題號'])).any()
        
        if c_mark.button("🌟 已標記" if is_m else "☆ 標記", key=f"m_{q_key}"):
            save_record("marks", user_id, subject, row['年度-期別'], row['題號'], "remove" if is_m else "add")
            st.rerun()

        render_content(row['題目內容'])
        if pd.notna(row['圖片路徑']) and ".png" in str(row['圖片路徑']):
            render_content(row['圖片路徑'])

        user_choice = st.radio("答案：", ["A", "B", "C", "D"], key=f"r_{q_key}", index=None, horizontal=True)
        for o in ["A", "B", "C", "D"]:
            st.write(f"**({o})**")
            render_content(row[f'選項 {o}'])

        btn_c1, btn_c2, _ = st.columns([1, 1, 3])
        if btn_c1.button("送出", key=f"s_{q_key}"):
            if user_choice == row['正確答案']:
                st.success("✅ 正確！")
                save_record("history", user_id, subject, row['年度-期別'], row['題號'], "add")
            else:
                st.error(f"❌ 錯誤！正確答案：{row['正確答案']}")
                save_record("wrong", user_id, subject, row['年度-期別'], row['題號'], "add")

        if btn_c2.button("💡 AI 詳解", key=f"a_{q_key}"):
            with st.spinner("AI 老師分析中..."):
                try:
                    p = f"請解析放射師國考題：\n題目：{row['題目內容']}\n(A){row['選項 A']}(B){row['選項 B']}(C){row['選項 C']}(D){row['選項 D']}\n正確答案是 {row['正確答案']}，請提供繁體中文詳解。"
                    resp = model.generate_content(p)
                    st.info(resp.text)
                except Exception as ai_e:
                    st.error(f"AI 連線失敗：{ai_e}")
